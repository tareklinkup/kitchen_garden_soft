...
<div class="col-xs-12">
	...
	<div class="form-horizontal">
		...
		<div class="form-group">
			...
			<div class="col-sm-8">
				<input type="text" id="Bank_name" name="Bank_name" placeholder="Bank Name" value="<?php echo $selected->Bank_name; ?>" class="col-xs-10 col-sm-4" />
				 <input name="Bank_SiNo" type="hidden" id="Bank_SiNo" class="required" value="<?php echo  $selected->Bank_SiNo; ?>" autofocus="" />
				<span id="msg1"></span>
			</div>
		</div>
...
<script type="text/javascript">
    function submit(){
        var Bank_SiNo= $("#Bank_SiNo").val();
        var Bank_name= $("#Bank_name").val();
        if(Bank_name==""){
            $("#msg1").html("Required Filed").css("color","red");
            return false;
        }
        var inputdata = 'Bank_SiNo='+Bank_SiNo+'&Bank_name='+Bank_name;
        alert(inputdata);
		var urldata = "<?php echo base_url() ?>updateBank";
        $.ajax({
            type: "POST",
            url: urldata,
            data: inputdata,
            success:function(data){                
                    ...
                    if((err)=="F"){
                        ...
                        alert("Save Success");
                        window.location.href='<?php echo base_url(); ?>bankEdit/'+Bank_SiNo;
...
<script type="text/javascript">
    function deleted(id){
        ...
        //alert(inputdata);
        var urldata = "<?php echo base_url() ?>Administrator/page/Bankdelete";
        $.ajax({
            type: "POST",
            url: urldata,
            data: inputdata,
